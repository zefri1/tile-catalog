...
class TileCatalog {
    ...
    initFilters() {
        ...
        const prices = this.state.products.map(p => p.price).filter(n => Number.isFinite(n));
        const maxPrice = prices.length ? Math.max(...prices) : 0;
        this.state.filters.priceMax = maxPrice;

        if (this.elements.priceRange) {
            this.elements.priceRange.max = String(maxPrice);
            this.elements.priceRange.value = String(maxPrice);
        }

        if (this.elements.priceMax) {
            this.elements.priceMax.value = String(maxPrice);
        }
    ...
    clearAllFilters() {
        ...
        // Сбрасываем ценовые фильтры (с защитой)
        const prices = this.state.products.map(p => p.price).filter(n => Number.isFinite(n));
        const maxPrice = prices.length ? Math.max(...prices) : 0;
    ...
    handleError(message, error) {
        ...
        this.hideLoadingScreen();
        this.notify('Произошла ошибка. Показаны демо-данные.');
    }
    ...
    notify(message) {
        try {
            ...
            Object.assign(n.style, {
                ...
                background: 'rgba(0,0,0,.8)', color: '#fff', padding: '10px 14px', borderRadius: '8px',
                fontSize: '14px', zIndex: 99999, boxShadow: '0 4px 16px rgba(0,0,0,.25)'
            });
...
// Создаем и инициализируем каталог (перенесено в main.js, но оставим для совместимости)
