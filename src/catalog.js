...
class TileCatalog {
  constructor() {
    ...
    this.filters = { search: '', brands: new Set(), colors: new Set(), countries: new Set(), surfaces: new Set(), uses: new Set(), structs: new Set(), categories: new Set(), priceMin: 0, priceMax: 1250 };
  ...
  async init() {
    ...
    try { await this.loadProducts(); this.initializeFilters(); this.bindEvents(); this.applyFilters(); }
  ...
  applyFilters() {
    ...
    switch (this.currentSort) { case 'price-asc': filtered.sort((a,b)=>a.price-b.price); break; case 'price-desc': filtered.sort((a,b)=>b.price-a.price); break; case 'name-asc': filtered.sort((a,b)=>a.name.localeCompare(b.name,'ru')); break; case 'name-desc': filtered.sort((a,b)=>b.name.localeCompare(a.name,'ru')); break; }

    this.filteredProducts = filtered; const counter=document.getElementById('results-count'); if(counter) counter.textContent = `Найдено товаров: ${this.filteredProducts.length}`; this.renderProducts();
  }

  renderProducts() {
    const grid = document.getElementById('products-grid'); const nr = document.getElementById('no-results'); if (!grid || !nr) return; if (!this.filteredProducts || this.filteredProducts.length === 0) { grid.innerHTML = ''; nr.classList.remove('hidden'); return; } nr.classList.add('hidden'); grid.innerHTML = ''; this.renderIndex = 0; const renderChunk = () => { const slice = this.filteredProducts.slice(this.renderIndex, this.renderIndex + this.batchSize); const html = slice.map((p, idx) => { const badge = p.inStock ? '<span class="status-badge status-in-stock">В НАЛИЧИИ</span>' : '<span class="status-badge status-on-demand">ПОД ЗАКАЗ</span>'; const img = p.image ? `<img src="${p.image}" alt="${p.name}" loading="lazy" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex'">` : ''; const ph = `<div class="product-placeholder" ${p.image ? 'style="display:none"' : ''}>🏠</div>`; const animationDelay = (this.renderIndex + idx) % 12 * 0.1; return `<article class="product-card" data-product-id="${p.id}" style="animation-delay: ${animationDelay}s;"><div class="product-image">${img}${ph}</div><div class="product-info"><h3 class="product-name">${p.name}</h3><p class="product-brand">${p.brand}</p><p class="product-color">${p.color}</p>${p.size ? `<span class="product-card__size">${p.size}</span>` : ''}<div class="product-price">${p.price.toLocaleString('ru-RU')} ₽</div><div class="product-status">${badge}</div></div></article>`; }).join(''); grid.insertAdjacentHTML('beforeend', html); this.renderIndex += slice.length; if (this.renderIndex < this.filteredProducts.length) { requestAnimationFrame(renderChunk); } else { this.attachCardClicks(); this.ensureLoadMore(); } }; requestAnimationFrame(renderChunk); }

  ensureLoadMore() { let btn = document.getElementById('load-more-btn'); if (!btn) { const container = document.createElement('div'); container.className = 'load-more-container'; container.innerHTML = `<button id=\"load-more-btn\" class=\"load-more-btn\">Показать ещё</button>`; const productsArea = document.querySelector('.products-area'); if (productsArea) productsArea.appendChild(container); btn = container.querySelector('#load-more-btn'); } if (btn) { btn.style.display = this.renderIndex >= this.filteredProducts.length ? 'none' : 'block'; btn.onclick = () => { this.batchSize += 60; this.renderProducts(); }; } }

  attachCardClicks() { const grid = document.getElementById('products-grid'); if (!grid) return; const cards = grid.querySelectorAll('.product-card'); if (cards) cards.forEach(card => { card.addEventListener('click', () => { const id = card.dataset.productId; const p = this.filteredProducts.find(x => x.id === id); if (p) this.openModal(p); }); }); }

  openModal(product) { const modal = document.getElementById('product-modal'); if (!modal) return; const modalTitle = document.getElementById('modal-title'); const modalBrand = document.getElementById('modal-brand'); const modalColor = document.getElementById('modal-color'); const modalPrice = document.getElementById('modal-price'); const modalDesc = document.getElementById('modal-desc'); const modalStatus = document.getElementById('modal-status'); if (modalTitle) modalTitle.textContent = product.name; if (modalBrand) modalBrand.textContent = product.brand; if (modalColor) modalColor.textContent = product.color; if (modalPrice) modalPrice.textContent = `${product.price.toLocaleString('ru-RU')} ₽`; if (modalDesc) modalDesc.textContent = product.description || 'Описание отсутствует'; if (modalStatus) modalStatus.textContent = product.inStock ? 'В наличии' : 'Под заказ'; const img = document.getElementById('modal-image'); const ph = document.getElementById('modal-image-ph'); if (product.image) { if (img) { img.src = product.image; img.style.display = 'block'; } if (ph) ph.style.display = 'none'; } else { if (img) img.style.display = 'none'; if (ph) ph.style.display = 'flex'; } modal.setAttribute('aria-hidden', 'false'); modal.classList.add('open'); document.body.classList.add('modal-open'); const closeBtn = modal.querySelector('.modal__close'); if (closeBtn) closeBtn.focus(); const closeModal = () => { modal.setAttribute('aria-hidden', 'true'); modal.classList.remove('open'); document.body.classList.remove('modal-open'); }; const modalCloseBtn = modal.querySelector('.modal__close'); const modalCloseBtnBottom = modal.querySelector('#modal-close-btn'); const modalBackdrop = modal.querySelector('.modal__backdrop'); if (modalCloseBtn) modalCloseBtn.onclick = closeModal; if (modalCloseBtnBottom) modalCloseBtnBottom.onclick = closeModal; if (modalBackdrop) modalBackdrop.onclick = closeModal; const handleEsc = (e) => { if (e.key === 'Escape') { closeModal(); document.removeEventListener('keydown', handleEsc); } }; document.addEventListener('keydown', handleEsc); }
}
...
let catalog = null;
if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded', () => { catalog = new TileCatalog(); catalog.init(); }); } else { catalog = new TileCatalog(); catalog.init(); }
